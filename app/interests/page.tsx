...
import qs from '@/data/interests.json'
import zh from '@/frontend/i18n/zh.json'
import en from '@/frontend/i18n/en.json'
import {useSearchParams} from 'next/navigation'
import {useState} from 'react'
type Q = {id:number; stem:{zh:string,en:string}; type:string; dimension:'R'|'I'|'A'|'S'|'E'|'C'}
...
function InterestsContent(){
  ...
  const submit=()=>{
    localStorage.setItem('talentai_interest_ans', JSON.stringify(ans))
    localStorage.setItem('talentai_lang', lang)
    location.href='/result?lang='+lang
  }
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:8,margin:'12px 0'}}>
        <button onClick={()=>choose(1)} style={{padding:'8px 12px', background: ans[step]===1?'#333':'#eee', color: ans[step]===1?'#fff':'#000'}}>{t.like}</button>
        <button onClick={()=>choose(0)} style={{padding:'8px 12px', background: ans[step]===0?'#333':'#eee', color: ans[step]===0?'#fff':'#000'}}>{t.dislike}</button>
      </div>
